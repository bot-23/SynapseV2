...
这是**已交付微信小程序与浏览器 Web 两个应用壳**的版本，核心学习规划链路、离线规则模式、本地资料检索、课程表避让、三层计划、复习闭环、**苏格拉底提示（三级递进、不给答案）**、**作业式计划（老师布置的任务 → 排进日程 → 盯着截止）**以及**图谱掌握度热力（学情诊断图）**在两大平台均已落地。桌面和 Android 仍只有架构规划，没有对应应用壳。
...
- core 单元测试、黄金样本、边界与压力测试：**121/121 通过**（含 22 句作业抽取评测：截止日/数量/科目 100%）
- core、小程序与 Web TypeScript 类型检查：通过
- vendor 导入边界：通过（core 对外导出 184 个，检查 35 个壳文件）
- Web Playwright 端到端测试：**25/25 通过**
...
| Web UI 自动化测试 | 🧪 | Playwright e2e 共 25 条，覆盖引导/对话/计划/资料导入（含 .md、批量上传、PDF 抽取）/资料构图/图谱（掌握度着色）/演示数据幂等与清空恢复/移动端导航/课程表（含节次）/作业（对话排期、打卡、倒计时、逾期风险、作业包扫码）/复习苏格拉底提示/我的；微信原生交互仍需真机验收 |
...
| 测试 | Vitest | ^3.0 | 121 个测试，含黄金样本回放与压力测试 |
...
| 门禁 | 命令 | 现状 |
| --- | --- | --- |
| 单元 + 回放 + 压力测试 | `npm test` | 121/121 通过 |
...
| 平台 | 状态 |
| --- | --- |
| 微信小程序（Taro） | 已交付主链路及资料结构化 / 批量导入 / 一键学习化 / Canvas 图谱（掌握度着色）/ 作业清单（逾期风险标）/ 复习苏格拉底提示 / 仪表盘 / 数据导出 / 演示数据；微信原生交互仍需真机验收，云开发 AI 仅自检 |
| Web（Vite + React） | 已交付主链路及资料结构化 / 批量导入 / PDF 抽取 / 一键学习化 / SVG 图谱（掌握度着色）/ 作业清单（逾期风险标、作业包二维码）/ 复习苏格拉底提示 / 仪表盘 / JSON 导出 / 演示数据，复用同一份 `@synapse/core`，25 条 e2e 通过 |
| 桌面（Tauri） | 待做 |
| Android | 待做 |
