...
这是**已交付微信小程序与浏览器 Web 两个应用壳**的版本，核心学习规划链路、离线规则模式、本地资料检索、课程表避让、三层计划、复习闭环、**作业式计划（老师布置的任务 → 排进日程 → 盯着截止）**以及**图谱掌握度热力（学情诊断图）**在两大平台均已落地。桌面和 Android 仍只有架构规划，没有对应应用壳。
...
- core 单元测试、黄金样本、边界与压力测试：**115/115 通过**（含 22 句作业抽取评测：截止日/数量/科目 100%）
- core、小程序与 Web TypeScript 类型检查：通过
- vendor 导入边界：通过（core 对外导出 178 个，检查 35 个壳文件）
- Web Playwright 端到端测试：**24/24 通过**
...
| 计划 | `pages/plan/index` | Tab | 今日（含「先学 5 分钟」一键启动）、短期、长期、复习四个视图 |
...
| 计划 | `pages/Plan.tsx` | 今日（含「先学 5 分钟」一键启动）、短期、长期、复习四个视图 |
