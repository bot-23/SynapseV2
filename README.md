...
这是**已交付微信小程序与浏览器 Web 两个应用壳**的版本，核心学习规划链路、离线规则模式、本地资料检索、课程表避让、三层计划、复习闭环、**苏格拉底提示（三级递进、不给答案）**、**作业式计划（老师布置的任务 → 排进日程 → 盯着截止）**、**图谱掌握度热力（学情诊断图）**以及**AI 学情周报（数字离线算、模型只写叙述）**在两大平台均已落地。桌面和 Android 仍只有架构规划，没有对应应用壳。
...
- core 单元测试、黄金样本、边界与压力测试：**126/126 通过**（含 22 句作业抽取评测：截止日/数量/科目 100%）
- core、小程序与 Web TypeScript 类型检查：通过
- vendor 导入边界：通过（core 对外导出 200 个，检查 35 个壳文件）
- Web Playwright 端到端测试：**26/26 通过**
...
| 最小启动行动 | ✅ | 今日卡顶部给出「先学 5 分钟：{第一条未完成任务}」，点一下走同一条打卡链路并把这笔用时记成 5 分钟——拖延的解法是把第一步缩到不可能失败，而不是把计划排得更满 |
...
| 我的 | `pages/mine/index` | Tab | 画像、科目、Key、仪表盘、AI 学情周报、课程表、资料库、作业清单、知识图谱、数据导出、演示数据与数据清理入口 |
...
| 我的 | `pages/Mine.tsx` | 画像、科目、Key、仪表盘、AI 学情周报、课程表/资料库入口、图谱入口、数据导出、演示数据、清空数据 |
...
| 知识图谱 | `pages/Graph.tsx` | SVG 环形布局、按掌握度着色 + 四色图例、点击节点查看说明与判定理由 |
...
| 风险预警口径 | 「可能逾期」只看这一条作业自己能否在剩余天数里按每日预算做完，**不计算多条作业互相抢时间**（那属于真排期，已由每日日程负责）。已逾期的不再重复报黄灯，只留红色「已逾期」。想要更严格的排期结论请看「未来排期」 |
| 最小启动口径 | 「先学 5 分钟」会勾掉该条任务、并把用时记为 5 分钟（走的是同一条打卡链路）。它记录的是「今天动起来了」，不等于这块内容真的学完了；数据上想看真实用时，就照常手动勾选 |
...
| 掌握度判定 | 四档口径写死在 core 常量里，保证「为什么是红的」永远可解释：复习卡 `repetitions>=3` 且平均 `ease>=2.2` → 已掌握；任一卡 `ease<1.8` 或 `lapses>=2` → 薄弱；其余 → 在学；没有关联卡的节点 → 未学。复习卡目前不存「最近一次评分」，所以用累计忘记次数 `lapses` 代替「最近评价偏难」这个信号。卡片与节点靠 `review_key(科目, 知识点)`（含别名）精确关联，对不上的卡片直接忽略 |
...
| Web UI 自动化测试 | 🧪 | Playwright e2e 共 26 条，覆盖引导/对话/计划/资料导入（含 .md、批量上传、PDF 抽取）/资料构图/图谱（掌握度着色）/演示数据幂等与清空恢复/移动端导航/课程表（含节次）/作业（对话排期、打卡、倒计时、逾期风险、作业包扫码）/复习苏格拉底提示/学情周报/我的；微信原生交互仍需真机验收 |
...
| 测试 | Vitest | ^3.0 | 126 个测试，含黄金样本回放与压力测试 |
...
| 门禁 | 命令 | 现状 |
| --- | --- | --- |
| 单元 + 回放 + 压力测试 | `npm test` | 126/126 通过 |
...
| 平台 | 状态 |
| --- | --- |
| 微信小程序（Taro） | 已交付主链路及资料结构化 / 批量导入 / 一键学习化 / Canvas 图谱（掌握度着色）/ 作业清单（逾期风险标）/ 复习苏格拉底提示 / 学情周报 / 仪表盘 / 数据导出 / 演示数据；微信原生交互仍需真机验收，云开发 AI 仅自检 |
| Web（Vite + React） | 已交付主链路及资料结构化 / 批量导入 / PDF 抽取 / 一键学习化 / SVG 图谱（掌握度着色）/ 作业清单（逾期风险标、作业包二维码）/ 复习苏格拉底提示 / 学情周报 / 仪表盘 / JSON 导出 / 演示数据，复用同一份 `@synapse/core`，26 条 e2e 通过 |
| 桌面（Tauri） | 待做 |
| Android | 待做 |
