...
Windows 上最省事的方式：**装好 [Node.js 18+](https://nodejs.org)，双击仓库根目录的 `start.cmd`**。
它会自动完成首次 `npm install`，起开发服务器，然后打开 http://localhost:5180 。（关掉那个「Synapse Web」黑窗口就是停服务。）

手动跑：

```bash
npm install
npm run dev --workspace @synapse/web      # Web 开发服务器 → http://localhost:5180
npm test                                  # core 单元/边界/压力测试
npm run typecheck                         # core + 小程序 + Web 类型检查
npm run e2e --workspace @synapse/web       # Web 端到端（需先起 dev server）
npm run sync:core                         # 改过 packages/core 后同步到小程序 vendor
```
...
7. **可视化**：Web 用 SVG 跑力导向收敛动画——每份资料连出的知识点自成一颗星，按学科分簇各占一块，点节点只看它与一跳邻居；小程序仍用 Canvas 渲染静态环形图谱。Web 的小球只按掌握度上色（资料中枢靠更大 + 一圈淡描边区分），小程序仍按分类着色。
...
不依赖任何外部冻结样本，全部断言都写在仓库内的测试里，改行为就得改测试——这是一次有意识的取舍：早期用于对齐旧 Python 实现的黄金样本已经退休，避免它反过来锁住产品迭代。
...
测试要跟着行为一起改，但**默认行为仍尽量稳定**：新能力优先做成显式开关，例如离线规划模式是 `offlinePlanFallback`（默认 `false`，即「没配 Key 就回一句固定话术」），由壳传 `true` 打开。这样「默认路径」和「离线增强路径」可以分别验证，不必二选一。
...
**重新构建后如果报 `X is not a function` 之类的加载期错误**（典型如 `n[e] is not a function`，且堆栈落在 webpack 的 require 里）：几乎都是工具侧缓存了上一次构建的分块。webpack 的模块 id 会随模块图变化而全量重排（例如删掉/新增一个源文件），旧分块和新分块混用时，页面分块要的模块在已加载的注册表里就找不到。先在工具里「清缓存 → 清除全部缓存」再编译；必要时删掉 `apps/miniprogram/dist` 重新构建。

Web e2e 优先使用仓库内的 Chromium；若本地尚未安装，则使用 Playwright 默认的浏览器安装位置。
